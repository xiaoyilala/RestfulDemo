...
import com.ice.good.lib.lib.restful.annotation.*
import com.ice.good.lib.lib.restful.annotation.Field
import java.lang.Exception
import java.lang.reflect.*

class MethodParse(private val baseUrl: String, method: Method) {
...
//        parseMethodParameters(method, args)
    ...
    fun newRequest(method: Method, args: Array<out Any>?): Request {
    ...
    private fun parseMethodParameters(method: Method, args: Array<Any>) {
        ...
        require(equals) { String.format("arguments annotations count %s don't match expect count %s", parameterAnnotations.size, args.size) }
    ...
    private fun parseMethodAnnotations(method: Method) {
        ...
        for(annotation in annotations){
            if(annotation is GET){
                ...
                val headersArr = annotation.value
                for(header in headersArr){
                    val colon = header.indexOf(":")
                    check(!(colon==0 || colon == -1 || colon == header.length-1)){
        ...
        require((httpMethod == Request.METHOD.GET) || (httpMethod == Request.METHOD.POST)
                || (httpMethod == Request.METHOD.PUT) || (httpMethod == Request.METHOD.DELETE) ) {
    ...
    private fun parseMethodReturnType(method: Method) {
        if(method.returnType != Call::class.java){
            throw IllegalStateException(String.format("method %s must be type of Call.class", method.name))
        }
        ...
        if(genericReturnType is ParameterizedType){
            ...
            require(validateGenericType(argument)) { String.format("method %s generic return type must not be an unknown type. " + method.name) }
            returnType = argument
        }else{
    ...
    companion object{
        fun parse(baseUrl: String, method: Method): MethodParse {
            return MethodParse(baseUrl, method)
        }
    }
}