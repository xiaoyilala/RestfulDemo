...
class TinyPngPTransform extends Transform {
    ...
    Set<QualifiedContent.ContentType> getInputTypes() {
        //接收的输入数据的类型
        //TransformManager.CONTENT_RESOURCES是指asserts目录下得资源，res里的一般使用task处理
        ...
        if (dir.isDirectory()) {
            dir.eachFileRecurse { file ->
                ...
                if (shouldModifyClass(filePath)) {
                    println("handleDirectoryIn file path:" + filePath)
...
//            if (!shouldModifyClass2(inputJarEntryName)) {
            ...
            println("handleDirectoryIn inputJarEntryName path:" + inputJarEntryName)
//            def ctClass = modifyClass2(inputStream)
    ...
    CtClass modifyClass2(InputStream is) {
        ...
        CtClass runnableImpl = classPool.makeClass("com.ice.good.lib.debug.RunnableImpl")
        ...
        runnableImpl.addConstructor(CtNewConstructor.make("public RunnableImpl(androidx.appcompat.widget.AppCompatImageView view, android.graphics.drawable.Drawable drawable) {\n" +
                "            this.view = view;\n" +
        ...
        runMethod.setBody("{int width = view.getWidth();\n" +
                ...
                "                    android.util.Log.e(\"LargeBitmapChecker\", \"bitmap:[\" + drawableWidth + \",\" + drawableHeight + \"],view:[\" + width + \",\" + height + \"],className:\" + this.view.getContext().getClass().getSimpleName());\n" +
                "                }\n" +
                "            }\n" +
                "            android.util.Log.e(\"LargeBitmapChecker\", \"bitmap:[\" + drawableWidth + \",\" + drawableHeight + \"],view:[\" + width + \",\" + height + \"],className:\" + this.view.getContext().getClass().getSimpleName());}")
        runnableImpl.addMethod(runMethod)
        runnableImpl.writeFile("lib_debug/build/intermediates/javac/debug/compileDebugJavaWithJavac/classes")
        runnableImpl.toClass()

        classPool.insertClassPath("com.ice.good.lib.debug.RunnableImpl")

        setImageDrawableMethod.insertBefore("if(drawable!=null){ post(new com.ice.good.lib.debug.RunnableImpl(this, drawable));}")
        return ctClass
    }
    ...
    CtClass modifyClass(InputStream is) {
        ...
        try{
            ...
            method.insertAfter("android.util.Log.e("+"\"TinyPngPTransform\","+ "\"" + message + "\""+");")
        }catch(Exception e){
    ...
    boolean shouldModifyClass(String filePath) {
        ...
        return filePath.endsWith("TestTransformActivity.class")
//        return (filePath.contains("com/ice/good/lib/lib/restful/demo")
